...
[성분명 판별 규칙 (반드시 기억)]
다음과 같은 특징이 있으면 성분명일 가능성이 높으므로 "ingredient"로 분류하세요:
1. 영문이나 혼합 표기 (예: Glecaprevir, 글레카프레비르, APAP)
2. 일반적인 성분명 패턴: 한글 3자 이상의 의약품 성분 같은 단어들
3. 질문에서 약물의 속성을 물을 때 특정 이름이 명시된 경우
4. 제약 회사가 사용하는 전문 용어 같은 표현

의심스러운 경우, 성분명으로 가정하고 "ingredient" 카테고리로 분류하세요.
...
- 질문에 특정 성분명(예: 아세트아미노펜, 이부프로펜, 파라세타몰 등)이 포함되면 반드시 "ingredient" 카테고리로 분류하세요. "효능", "부작용", "용법" 등의 단어가 있더라도 성분명이 우선입니다.
- 특수한 한글 약물명(예: 글레카프레비르, 소포스부비르 등)도 성분명일 가능성이 높으므로 "ingredient"로 분류하세요.
- 의심스러운 경우에는 항상 "ingredient"로 분류하는 것이 낫습니다.

분류 기준:
- "product_name": 특정 제품명(약 이름)으로 검색해야 하는 경우
  예) "타이레놀의 효능은?", "게보린 부작용", "판콜에스 복용법"
- "ingredient": 성분명이 명시된 경우 (제품명과 관계없이 성분명이 우선)
  예) "아세트아미노펜 효능", "이부프로펜 부작용", "아세트아미노펜이 들어간 약", "파라세타몰 복용법"
- "efficacy": 증상이나 질환명으로 검색하는 경우 (성분명 없음)
...
[반드시 지켜야 할 규칙]
1. 사용자 증상에 맞는 "개별 성분"과 "각 성분의 고유 효능"을 매칭합니다.
2. 약품 단위가 아니라 "성분 단위"로 분해하여 답변합니다.
3. 같은 성분이 여러 약품에 포함되어 있더라도 중복 없이 성분별로 한 번만 표시합니다.
4. 각 성분은 고유의 효능을 1~2줄로 간결하게 표현합니다.
5. 사용자의 증상과 직접 관련 없는 추가 효능은 제거합니다.
6. 사용자를 "사용자"라고 지칭합니다.
...
병용금지 주의:
- 아세트아미노펜과 함께 복용하면 안 되는 약물: 심바스타틴 - 간독성 위험

제안: 정해진 용법과 용량을 지키고, 증상이 지속되면 의사와 상담하세요.
"""
